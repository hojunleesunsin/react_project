...
import { GridRow, SearchFilter, searchP00017Action } from './actions';
...
export default function Page() {
  ...
  useEffect(() => {
    ...
    const runSearch = async () => {
      ...
      setIsLoading(true);
      try {
        const result = await searchP00017Action(appliedFilter);
        if (cancelled) return;
        setColumnOrder(result.columns.map((col) => col.name));
        setRows(result.rows);
    ...
    <main className="min-h-screen bg-slate-50 p-6 text-slate-900">
      <div className="mx-auto max-w-6xl space-y-4">
        <header className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          <h1 className="text-2xl font-bold">P00017 - USER_TAB_COLUMNS 기반 동적 컬럼</h1>
          <p className="mt-1 text-sm text-slate-600">
            컬럼 메타데이터를 먼저 읽고, MST_EMPLOYEE를 SELECT *로 조회해 전체 컬럼을 렌더링합니다.
          </p>
        </header>
